...
def make_peloton(n, parsed_spec):
	...
	one_cycles = [ OneCycle(p + SYMBOLS[n]) for p in peloton ]

	return one_cycles

def instances_from_peloton(n, peloton):
	one_cycles_set = set(peloton)

	for three_cycle in three_cycles(n):
		s = set(three_cycle.one_cycles())
		if len(s.intersection(one_cycles_set)) == 1:
			m = [list(s.difference(one_cycles_set))]
			secondary = set()
			all_one_cycles = s.union(one_cycles_set)

			for partial_two_cycle in partial_two_cycles(n):
				if not partial_two_cycle.intersection(all_one_cycles):
...
for (matrix, secondary) in instances_from_peloton(n, peloton):
	for row in matrix: print row, len([ x for x in row if x not in secondary ])

	all_one_cycles = set()
	for row in matrix:
		all_one_cycles.update(set(row))

	print "Searching (%d)..." % (len(all_one_cycles.difference(secondary)))
